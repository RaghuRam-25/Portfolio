...
const makeCrudController = ({
  ...
  const list = asyncHandler(async (req, res) => {
    const baseFilter = req.user?.role === 'admin' ? notDeleted : { ...notDeleted, ...publicFilter };
  ...
  const getOne = asyncHandler(async (req, res) => {
    const row = await model.findOne({ _id: req.params.id, deletedAt: null }).populate(populate);
